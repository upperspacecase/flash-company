...
export function DemoWorkspace({ plan }: { plan: "free" | "full" }) {
  ...
  const cohort = isFree ? COHORT.slice(0, 3) : COHORT;
  ...
  // Free plan stops at the convergence map; later phases are locked.
  const go = (i: number) => setPhase(isFree && i >= 2 ? 1 : i);

  return (
    <div className="flex min-h-screen flex-col bg-slate-50/70">
      <Header phase={phase} plan={plan} cohort={cohort} />
      <Timeline phase={phase} onJump={go} plan={plan} />
      <main className="mx-auto w-full max-w-[1500px] flex-1 px-5 py-6">
        {phase === 0 && <CohortPhase plan={plan} cohort={cohort} onNext={() => setPhase(1)} />}
        {phase === 1 && <ConvergencePhase plan={plan} cohort={cohort} dumps={dumps} onDump={() => setDumps((d) => Math.min(DUMPS.length, d + 1))} onNext={() => setPhase(2)} />}
        {!isFree && phase === 2 && <FormationPhase hypId={hypId} onSelect={setHypId} statuses={statuses} setStatuses={setStatuses} onNext={() => setPhase(3)} />}
        {!isFree && phase === 3 && <OutputPhase recorded={recorded} onRecord={(id) => setRecorded((r) => ({ ...r, [id]: !r[id] }))} onNext={() => setPhase(4)} />}
        {!isFree && phase === 4 && <FollowThroughPhase active={checkpoint} onSelect={setCheckpoint} recorded={recorded} />}
      </main>
    </div>
  );
}

function Header({ phase, plan, cohort }: { phase: number; plan: "free" | "full"; cohort: Member[] }) {
      ...
      <div className="mx-auto flex w-full max-w-[1500px] items-center justify-between gap-4 px-5 py-3">
        ...
        <div className="flex shrink-0 items-center gap-3">
          <span className="hidden items-center gap-2 rounded-full bg-sage-tint px-3 py-1.5 text-xs font-semibold text-sage-dark sm:flex">
            <Icon name="clock" className="h-3.5 w-3.5" /> {isFree ? "Free · one-day session" : `${SPRINT.windowHours}h sprint · ${p.day}`}
          </span>
            ...
            <li key={p.id} className="flex flex-1 items-center gap-2">
              <button disabled={locked} onClick={() => !locked && onJump(i)} className={`flex flex-1 items-center gap-3 rounded-xl border px-3 py-2 text-left transition-colors ${active ? "border-sage bg-sage-tint/40" : locked ? "cursor-not-allowed border-transparent opacity-50" : "border-transparent hover:bg-slate-50"}`}>
                ...
                <span className="min-w-0">
                  <span className="block whitespace-nowrap text-[10px] font-bold uppercase tracking-wide text-slate-400">{locked ? "Full protocol" : p.day}</span>
...
function CohortPhase({ plan, cohort, onNext }: { plan: "free" | "full"; cohort: Member[]; onNext: () => void }) {
  const isFree = plan === "free";
  return (
    <Columns
      left={
        <div className="space-y-4">
          ...
          <Card>
            <p className="flex items-center gap-2 font-bold text-foreground"><Icon name="clock" className="h-4 w-4 text-sage" /> {isFree ? "A single session" : "Time-boxed on purpose"}</p>
            <p className="mt-1.5 text-sm text-slate-600">{isFree ? "Converge today. Upgrade for the full 72-hour protocol and the venture output." : `A fixed ${SPRINT.windowHours}-hour window forces action — the constraint is the feature.`}</p>
            <p className="mt-3 rounded-lg bg-sage-tint/40 px-3 py-2 text-center text-lg font-bold text-sage-dark">{isFree ? "One-day session" : "71:48 left"}</p>
          </Card>
        </div>
      }
      center={
        <Card className="p-6">
          <CenterHead title="Open the sprint" sub={isFree ? "Invite up to 3 people you trust. One session — converge today." : "Invite 3–5 people you trust. Each contributes individually — no group chat."} />
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-slate-200 p-4">
              <p className="flex items-center gap-2 text-sm font-bold text-foreground"><Icon name="coins" className="h-4 w-4 text-sage" /> {isFree ? "Free" : "Buy-in"}</p>
              <p className="mt-1.5 text-sm text-slate-600">{isFree ? "Basic convergence map, no buy-in. Upgrade for the full venture output." : `${SPRINT.buyInTotal} total into the kitty (${SPRINT.buyInPer} each). Everyone has skin in the game.`}</p>
            </div>
            <div className="rounded-xl border border-slate-200 p-4">
        ...
        <div className="space-y-4">
          ...
          <Card className="bg-sage-tint/30">
            ...
            <ul className="space-y-1.5">
              {isFree
                ? [{ label: "Up to 3 people invited", done: true }, { label: "Free one-day session", done: true }, { label: "Ready to converge", done: true }].map((r) => <CheckRow key={r.label} label={r.label} done={r.done} />)
                : COHORT_READINESS.map((r) => <CheckRow key={r.label} label={r.label} done={r.done} />)}
            </ul>
          </Card>
          {isFree && <p className="px-1 text-xs text-slate-400">Free sprints are saved to the public gallery.</p>}
        </div>
      }
    />
  );
...
function ConvergencePhase({ plan, cohort, dumps, onDump, onNext }: { plan: "free" | "full"; cohort: Member[]; dumps: number; onDump: () => void; onNext: () => void }) {
  const isFree = plan === "free";
    ...
    <Columns
      left={
        <div className="space-y-4">
          ...
          <p className="px-1 text-xs text-slate-400">{isFree ? "Each person dumps separately in the session." : "Each person dumps separately, async, across the 3 days."}</p>
          {cohort.map((m) => {
      ...
      center={
        <Card className="flex h-full flex-col p-6">
          <CenterHead title="Convergence" sub="Dump anything — no structure. The agent reads across everyone's input." />
          ...
          <div className="mt-5 border-t border-slate-100 pt-4">
            {!done ? (
              ...
              <div className="rounded-xl border border-sage/30 bg-sage-tint/30 p-4">
                <p className="font-semibold text-foreground">That&rsquo;s your convergence map.</p>
                <p className="mt-1 text-sm text-slate-600">The full protocol turns it into a committed venture — formation, the birth certificate, and the 30-day follow-up.</p>
                <div className="mt-3 flex flex-wrap items-center gap-4">
                  <a href="/demo" className="inline-flex h-11 items-center gap-2 rounded-xl bg-sage px-5 text-sm font-bold text-white transition-colors hover:bg-sage-dark"><Icon name="bolt" className="h-4 w-4" /> Unlock the full protocol</a>
